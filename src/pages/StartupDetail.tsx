...
function ImpactPL({ startup }: { startup: typeof STARTUPS[0] }) {
  const ext = startup.extended;

  // Aggregate annual from financials (last 6 months for simplicity)
  const last6 = ext.financials.slice(-6);
  const totalRevenue = last6.reduce((s, m) => s + m.revenue, 0);
  const totalCosts = last6.reduce((s, m) => s + m.costs, 0);
  const reportedProfit = totalRevenue - totalCosts;

  // Environmental externalities
  const co2Tonnes = startup.carbonOffset; // using as proxy for emissions
  const energyCostEstimate = startup.sustainability.energyEfficiency.score < 20 ? 8000 : 3000;
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <UITooltip>
          ...
          <TooltipContent className="max-w-xs text-xs">
            Impact-weighted accounting subtracts estimated environmental costs from reported profits to reveal true sustainable profitability.
          </TooltipContent>
        </UITooltip>
      </div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData} barGap={4}>
            ...
            <Tooltip
              ...
              formatter={(v: number, name: string) => [fmt(v), name === 'traditional' ? 'Traditional' : 'Impact-Adjusted']}
            />

...
const inputCls = 'w-full rounded-lg border border-border bg-card px-4 py-2.5 text-foreground focus:border-primary focus:ring-1 focus:ring-primary outline-none transition';
...
const initForm: FormData = {
  name: '', description: '', category: 'DeFi', blockchain: 'Base', website: '', foundedDate: '',
...
function Toggle({ on, onToggle, label }: { on: boolean; onToggle: () => void; label: string }) {
  return (
    <button onClick={onToggle} className="flex w-full items-center justify-between rounded-lg border border-border bg-card p-4 transition hover:bg-secondary">
...
function DistSlider({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <div>
  ...
  if (success) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-lg flex-col items-center justify-center px-4 py-20 text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground">Confirmed on Base!</h2>

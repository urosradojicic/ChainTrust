          ...
          <tbody>
            {entries.map((e) => (
              <tr key={e.id} className="border-b border-border/50 hover:bg-muted/20 transition">
                <td className="px-4 py-3 font-medium text-foreground capitalize">{e.field_changed.replace(/_/g, ' ')}</td>
                <td className="px-4 py-3 font-mono text-xs text-muted-foreground max-w-[120px] truncate">{e.old_value || '—'}</td>
                <td className="px-4 py-3 font-mono text-xs text-foreground max-w-[120px] truncate">{e.new_value || '—'}</td>
                <td className="px-4 py-3">
                  <a href={`https://sepolia.etherscan.io/tx/${e.tx_hash}`} target="_blank" rel="noopener noreferrer"

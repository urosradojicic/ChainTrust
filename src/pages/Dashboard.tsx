...
import { STARTUPS, ACTIVITY_FEED, CATEGORIES } from '@/lib/mock-data';
...
type SortKey = 'mrr' | 'growth' | 'trustScore' | 'users';
...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      const aVal = sortKey === 'trustScore' ? a.trustScore : a[sortKey];
      const bVal = sortKey === 'trustScore' ? b.trustScore : b[sortKey];
              ...
              <div className="flex rounded-lg border border-border">
                {(['grid', 'table'] as const).map(v => (
                  <button
                    ...
                    className={`p-1.5 transition ${viewMode === v ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'} ${v === 'grid' ? 'rounded-l-lg' : 'rounded-r-lg'}`}
                  >

...
const EVENT_GENERATORS: ((block: number) => string)[] = [
  (block) => {
    ...
    return `Block #${block.toLocaleString()} — ${s.name} revenue updated: $${(s.mrr / 1000).toFixed(0)}K → $${(newMrr / 1000).toFixed(0)}K`;
  },
  (block) => {
    const s = STARTUP_DATA[Math.floor(Math.random() * STARTUP_DATA.length)];
    const tons = Math.floor(Math.random() * 20) + 2;
    return `Block #${block.toLocaleString()} — ${s.name} carbon offset verified: +${tons}t CO₂`;
  },
  (block) => {
    const s = STARTUP_DATA[Math.floor(Math.random() * STARTUP_DATA.length)];
    const delta = Math.floor(Math.random() * 50) + 5;
    return `Block #${block.toLocaleString()} — ${s.name} user count oracle updated: ${(s.users + delta).toLocaleString()} wallets`;
  },
  (block) => {
    const s = STARTUP_DATA[Math.floor(Math.random() * STARTUP_DATA.length)];
  ...
  (block) => {
    const s = STARTUP_DATA[Math.floor(Math.random() * STARTUP_DATA.length)];
    return `Block #${block.toLocaleString()} — ${s.name} sustainability pledge recorded on-chain`;
  },
];
